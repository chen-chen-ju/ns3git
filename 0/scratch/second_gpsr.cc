...
int select[All_num];
int count=0;
RngSeedManager::SetSeed (66); // Changes seed from default of 1 to 3
...
*/
  ...
  for (int i=0; i<source_num; i++)
 {
  ...
  AnimationInterface anim("second_gpsr0.xml"); 
  for (int i=0;i<200;i++)
  {
  ...
  monitor->CheckForLostPackets(Time (Seconds (5)));
  ...
  NS_LOG_INFO("flow	   "<< "Lost Packets	  " <<"delay	"<<"Tx Packets	"<<"Rx Packets	"<<std::endl);

  for (int j=0; j<source_num; j++)
  {
      NS_LOG_INFO(select[2*j+1] <<"  to  "<<select[2*j]<<"	"<<lost.at(j)<<"	"<< delay.at(j) <<"	"<<Tx.at(j)<<"	"<<Rx.at(j)<<std::endl);
  }
  Simulator::Destroy ();
  return 0;
}
