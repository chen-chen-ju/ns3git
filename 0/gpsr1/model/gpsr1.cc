...
#define GPSR1_LS_GOD 0

#define GPSR1_LS_RLS 1



namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("Gpsr1RoutingProtocol");
namespace gpsr1 {
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::gpsr1::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
#define GPSR1_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-GPSR_MAXJITTER)-GPSR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (UniformRandomVariable ().GetValue (-GPSR1_MAXJITTER, GPSR1_MAXJITTER))) 
#define FIRST_JITTER (Seconds (UniformRandomVariable ().GetValue (0, GPSR1_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4
...
const uint32_t RoutingProtocol::GPSR1_PORT = 666;
...
{
  static TypeId tid = TypeId ("ns3::gpsr1::RoutingProtocol")
    ...
    .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                   EnumValue (GPSR1_LS_GOD),
                   MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                   MakeEnumChecker (GPSR1_LS_GOD, "GOD",
                                    GPSR1_LS_RLS, "RLS"))
...
NS_LOG_FUNCTION (this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
  if (m_socketAddresses.empty ())
    {
      NS_LOG_LOGIC ("No gpsr1 interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  NS_ASSERT (p != 0);
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      TypeHeader tHeader (GPSR1TYPE_POS);
      packet->RemoveHeader (tHeader);
      if (!tHeader.IsValid ())
        {
          NS_LOG_DEBUG ("GPSR1 message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
          return false;
        }
      
      if (tHeader.Get () == GPSR1TYPE_POS)
        {
          PositionHeader phdr;
        ...
        while(m_queue.Dequeue (dst, queueEntry))
          {
            ...
            if (!tHeader.IsValid ())
              {
                NS_LOG_DEBUG ("GPSR1 message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
                return false;     // drop
              }
            if (tHeader.Get () == GPSR1TYPE_POS)
              {
            ...
            PositionHeader posHeader (Position.x, Position.y, Position.z, updated, myPos.x, myPos.y,myPos.z, (uint8_t) 1, Position.x, Position.y,Position.z, (uint8_t) 0, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0); 
...
RoutingProtocol::RecoveryMode(Ipv4Address dst, Ptr<Packet> p, UnicastForwardCallback ucb, Ipv4Header header){
  ...
  PositionHeader posHeader (Position.x, Position.y, Position.z, updated, recPos.x, recPos.y,recPos.z, (uint8_t) 1, myPos.x, myPos.y,myPos.z, (uint8_t) 0, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0); 
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
  ...
  if (l3->GetNAddresses (interface) > 1)
    {
      NS_LOG_WARN ("GPSR1 does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvGPSR1, this));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), GPSR1_PORT));
...
void
RoutingProtocol::RecvGPSR1 (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address sourceAddress;
  Ptr<Packet> packet = socket->RecvFrom (sourceAddress);

  TypeHeader tHeader (GPSR1TYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("GPSR1 message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
...
{
  TableEntry rt(Pos,Vector(0,0,0),Vector(0,0,0),Simulator::Now ());
  m_neighbors.AddEntry (sender, rt, HelloInterval);
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses ((interface) == 1))
    ...
    {
      NS_LOG_LOGIC ("GPSR1 does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in GPSR1 operation");
    }
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      socket->SendTo (packet, 0, InetSocketAddress (destination, GPSR1_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid GPSR1 source address not found");
...
RoutingProtocol::AddHeaders (Ptr<Packet> p, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route)
{
  ...
  PositionHeader posHeader (positionX, positionY,positionZ,  hdrTime, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y,myPos.z, (uint8_t) 0, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0); 
  p->AddHeader (posHeader);
  TypeHeader tHeader (GPSR1TYPE_POS);
  p->AddHeader (tHeader);
...
*/
      ...
      if (nextHop != Ipv4Address::GetZero ())
        {
          PositionHeader posHeader (Position.x, Position.y, Position.z, updated, (uint64_t) 0, (uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y,myPos.z, (uint8_t) 0, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0);
          p->AddHeader (posHeader);
          p->AddHeader (tHeader);
