      ...
      while (!file.eof () )
        {
          ...
          if (pr.tokens.size () != 4 && pr.tokens.size () != 7 && pr.tokens.size () != 9)
            {
...
ParseNs2Line (const std::string& str)
{
  ...
  if ( (tokensLength == 7 || tokensLength == 9)
...
IsSchedMobilityPos (ParseResult pr)
{
  ...
  return pr.tokens.size () == 9 && pr.tokens[0] == NS2_NS_SCH && pr.tokens[1] == NS2_AT
         ...
         && pr.has_dval[2] && pr.has_dval[5] && pr.has_dval[6] && pr.has_dval[8]
  ...
  if (speed > 0)
    {
      ...
      // Set the Values
      Simulator::Schedule (Seconds (at), &ConstantVelocityMobilityModel::SetPosition, model, Vector (retval.m_finalPosition.x, retval.m_finalPosition.y, retval.m_finalPosition.z));
      ...
      retval.m_finalPosition.z=zFinalPosition;
