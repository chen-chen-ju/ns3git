...
#define GPSR0_LS_GOD 0

#define GPSR0_LS_RLS 1



namespace ns3 {
NS_LOG_COMPONENT_DEFINE ("Gpsr0RoutingProtocol");
namespace gpsr0 {
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::gpsr0::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
#define GPSR0_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-GPSR_MAXJITTER)-GPSR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (UniformRandomVariable ().GetValue (-GPSR0_MAXJITTER, GPSR0_MAXJITTER))) 
#define FIRST_JITTER (Seconds (UniformRandomVariable ().GetValue (0, GPSR0_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4
...
const uint32_t RoutingProtocol::GPSR0_PORT = 666;
...
{
  static TypeId tid = TypeId ("ns3::gpsr0::RoutingProtocol")
    ...
    .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                   EnumValue (GPSR0_LS_GOD),
                   MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                   MakeEnumChecker (GPSR0_LS_GOD, "GOD",
                                    GPSR0_LS_RLS, "RLS"))
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      TypeHeader tHeader (GPSR0TYPE_POS);
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
  ...
  if(m_neighbors.isNeighbour (dst))
    ...
    nextHop = m_neighbors.BestNeighbor (dst, dstPos, myPos, m_ipv4->GetObject<MobilityModel> ()->GetVelocity ());
        ...
        while(m_queue.Dequeue (dst, queueEntry))
          {
            ...
            PositionHeader posHeader (Position.x, Position.y, Position.z, updated, myPos.x, myPos.y,myPos.z, (uint8_t) 1, Position.x, Position.y,Position.z, m_ipv4->GetAddress (1, 0).GetLocal ()); 
  ...
  m_neighbors.Setpacketcounts(nextHop,(uint32_t)1);
  return true;
 // }
/*
  else
...
RoutingProtocol::RecoveryMode(Ipv4Address dst, Ptr<Packet> p, UnicastForwardCallback ucb, Ipv4Header header){
  ...
  PositionHeader posHeader (Position.x, Position.y, Position.z, updated, recPos.x, recPos.y,recPos.z, (uint8_t) 1, myPos.x, myPos.y,myPos.z, a); 
  p->AddHeader (posHeader);
  p->AddHeader (tHeader);



  Ipv4Address nextHop = m_neighbors.BestNeighbor (dst, previousHop, myPos, m_ipv4->GetObject<MobilityModel> ()->GetVelocity ()); 
  if (nextHop == Ipv4Address::GetZero ())
    {
      return;
    }
  ...
  if (tHeader.Get () == GPSR0TYPE_POS)
{
  m_neighbors.Settable2(dst,nextHop);
  SendReply(a,(uint32_t) 1);
  m_neighbors.Setpacketcounts(nextHop,(uint32_t)1);
}
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvGPSR0, this));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), GPSR0_PORT));
...
void
RoutingProtocol::RecvGPSR0 (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address sourceAddress;
  Ptr<Packet> packet = socket->RecvFrom (sourceAddress);

  TypeHeader tHeader (GPSR0TYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("GPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
  ...
  if (tHeader.Get ()==GPSR0TYPE_HELLO)
  {
        HelloHeader hdr;
        ...
        Velocity.x = hdr.GetOriginVx ();
        Velocity.y = hdr.GetOriginVy ();
        Velocity.z = hdr.GetOriginVz ();
        Ipv4Address a = hdr.Geta();
        double trust = hdr.Gettrust();
        trust=trust/10000;
        ...
        m_neighbors.updatetrust(sender, a, trust);
   }


  if (tHeader.Get ()==GPSR0TYPE_ACK)
  {    
        AckHeader hdr;
        packet->RemoveHeader (hdr);
        uint32_t num = hdr.Getnum ();
 
        InetSocketAddress inetSourceAddr = InetSocketAddress::ConvertFrom (sourceAddress);
        Ipv4Address sender0 = inetSourceAddr.GetIpv4 ();
        //Ipv4Address receiver = m_socketAddresses[socket].GetLocal ();
        Ipv4Address sender = hdr.GetAdress();
        if (sender == sender0 )
        {m_neighbors.Setpacketsuc(sender, num);}

        //std::cout<<"counts:"<<(int)m_neighbors.Getpacketcounts(sender, 0)<<std::endl;
  }
/*
  case GPSR0TYPE_ACK:
       {
        AckHeader hdr;
        uint8_t num = hdr.Getnum ();
        packet->RemoveHeader (hdr);
        InetSocketAddress inetSourceAddr = InetSocketAddress::ConvertFrom (sourceAddress);
        Ipv4Address sender = inetSourceAddr.GetIpv4 ();
        //Ipv4Address receiver = m_socketAddresses[socket].GetLocal ();
        m_neighbors.Setpacketcounts(sender,uint32_t(num));
        break;
       }
  case GPSR0TYPE_POS:
       {
...
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver, Vector Pos, Vector Vel)
{
  ...
  //TableEntry rt(Pos,times,counts,v,Simulator::Now ());
  TableEntry rt(Pos,0, 0, 0, 0.5, Vel,Simulator::Now (),Simulator::Now ()); 
  m_neighbors.AddEntry (sender, rt);
/*
  if (receiver==Ipv4Address("10.1.1.1")&&sender==Ipv4Address("10.1.1.2"))
{
  std::cout<<"2 "<<std::endl;
  for (uint8_t i=0 ;i<8 ;i++)
  {
  double vel=m_neighbors.Gettrust(Ipv4Address("10.1.1.1"),i);
  std::cout<<"a:"<<vel<<std::endl;
  }
}
*/
}
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      HelloHeader helloHeader (((uint64_t) positionX),((uint64_t) positionY),((uint64_t) positionZ),((uint64_t) velocityX),((uint64_t) velocityY),((uint64_t) velocityZ), Ipv4Address::GetZero (), ((uint64_t) 0.5));
      ...
      socket->SendTo (packet, 0, InetSocketAddress (destination, GPSR0_PORT));

    }
*/

  std::map<Ipv4Address, double> alltrust;
  m_neighbors.GetListOfAlltrust(alltrust);
if (!alltrust.empty())
{
for (std::map<Ipv4Address, double>::const_iterator i = alltrust.begin (); i != alltrust.end (); ++i)
{
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      Ptr<Socket> socket = j->first;
      Ipv4InterfaceAddress iface = j->second;
      double value0 = 10000* i->second;
      HelloHeader helloHeader (((uint64_t) positionX),((uint64_t) positionY),((uint64_t) positionZ),((uint64_t) velocityX),((uint64_t) velocityY),((uint64_t) velocityZ), i->first, ((uint64_t)value0));
...
else
  {
    for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      HelloHeader helloHeader (((uint64_t) positionX),((uint64_t) positionY),((uint64_t) positionZ),((uint64_t) velocityX),((uint64_t) velocityY),((uint64_t) velocityZ), Ipv4Address::GetZero (), ((uint64_t) 5000));
...
RoutingProtocol::AddHeaders (Ptr<Packet> p, Ipv4Address source, Ipv4Address destination, uint8_t protocol, Ptr<Ipv4Route> route)
{
  ...
  if(m_neighbors.isNeighbour (destination))
    ...
    {
      nextHop = m_neighbors.BestNeighbor (destination, m_locationService->GetPosition (destination), myPos, m_ipv4->GetObject<MobilityModel> ()->GetVelocity ());
    }
  ...
  PositionHeader posHeader (positionX, positionY,positionZ,  hdrTime, (uint64_t) 0,(uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y,myPos.z,m_ipv4->GetAddress (1, 0).GetLocal ()); 
  p->AddHeader (posHeader);
  TypeHeader tHeader (GPSR0TYPE_POS);
  p->AddHeader (tHeader);
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  if(m_neighbors.isNeighbour (dst))
  ...
  else
    {
      ...
      nextHop = m_neighbors.BestNeighbor (dst,Position, myPos,m_ipv4->GetObject<MobilityModel> ()->GetVelocity ());
      if (nextHop != Ipv4Address::GetZero ())
        {
            //std::ostringstream addrOss;
            //m_ipv4->GetAddress (1, 0).GetLocal ().Print(addrOss);
        ...
        if (tHeader.Get () == GPSR0TYPE_POS)
        {
          ...
          PositionHeader posHeader (Position.x, Position.y, Position.z, updated, (uint64_t) 0, (uint64_t) 0, (uint64_t) 0, (uint8_t) 0, myPos.x, myPos.y,myPos.z,m_ipv4->GetAddress (1, 0).GetLocal ());
          p->AddHeader (posHeader);
          p->AddHeader (tHeader);
...
*/
 ...
 void
 RoutingProtocol::SendReply (Ipv4Address neighbor, uint32_t val)
 {
   ...
   for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      Ptr<Socket> socket = j->first;
      AckHeader h(val, m_ipv4->GetAddress (1, 0).GetLocal ());
      Ptr<Packet> packet = Create<Packet> ();
      packet->AddHeader (h);
      TypeHeader tHeader (GPSR0TYPE_ACK);
      packet->AddHeader (tHeader);
      socket->SendTo (packet, 0, InetSocketAddress (neighbor, GPSR0_PORT));
